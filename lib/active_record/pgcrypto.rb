      ...
      def has_encrypted_attributes(*args)
        ...
        attrs.each do |a|
          ...
          define_method "#{a}=" do |value|
            ...
            # remaining the same
            if self.send(hashed_attr) == digest_value
              return value
            end
          ...
          scope "find_by_#{a}".to_sym, ->(val) {
            if val.nil?
              ...
              where(self.arel_table["hashed_#{a}".to_sym]
                      .eq(Arel::Nodes::NamedFunction.new('crypt',
                                                         [Arel::Nodes::BindParam.new,
                                                          Arel::Nodes::BindParam.new]))
                   ).tap{ |arel| arel.bind_values += [[ nil, val], [nil, ActiveRecord::Pgcrypto.configuration.salt ]]}
